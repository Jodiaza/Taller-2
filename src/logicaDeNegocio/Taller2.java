...
public class Taller2 implements Serializable{
    ...
    public static void bienvenida() {
    	...
    	System.out.println("Bienvenido al banco Javaliste!\nPorfavor  escoja una de las"
    			...
    			+ "\n7. Ver el ranquing de malditos burgueses.");
	   ...
	   if(flujo.leerArray()!=null) { 	
	    	if(flujo.leerArray().size()!=0) {
		    	...
		    	for(String cuenta:cuentas) {
		    		sistema.getListaClientes().put(cuenta, flujo.leerObjeto(cuenta+".obj"));
		    	}
	    	}else {}
	   }else {}
    }
			...
			if(clave.equals(cliente.getClave())){
				if(cliente.getCuenta().getClass().equals(Ahorro.class)) {
					...
					if(cliente.getCuenta().getSaldo()>10000000) {
						...
						System.out.println("Su cuenta bancaria es de tipo cdt. Hasta no tener"
								+ " mas de 10000000$ en su cuenta no podrá retirarlos");
						Taller2.bienvenida();
					}
				}
			}else {
